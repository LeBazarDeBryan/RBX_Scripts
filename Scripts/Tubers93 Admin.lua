--this script was not stolen!
--made by tubers93 follow me on tiktok at @user463937336292
...
FindCmd.PlaceholderText = "find command"
FindCmd.Text = ""
...
CmdHandler.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
...
CmdText.Parent = nil
CmdText.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
...
Minimum.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
...
Background2.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
...
local Cmds = {
	"Version "..Versions,
	"script made by tubers",
    "--commands--",
	"cmd / cmds - shows commands",
	"rejoin / rj - rejoin the server",
	"leave / te leave the server",
	"admin / giveadmin [plr] / give admin to player",
	"unadmin / remove admin removes admin",
	"kill [plr] - kills player",
	"killall - kills all",
	"killguard / kills all guards",
	"killinmate / kills all inmates",
	"killcriminal / kills all criminals",
	"tase [plr] - tase player",
	"loopkill / loopkills [plr] - loopkill player",
	"unloopkill / unloopkills [plr] -  stops loopkill",
	"loopkillall / loopkillsall - loopkills all",
	"loopkillguard / loopkillsguard - loopkills all guards",
	"loopkillinmate / loopkillinmates - loopkills all inmates",
	"loopkillcriminal / loopkillcriminals - loopkills all criminals",
	"unloopkillall / unloopkillsall - stops all loopkill ",
	...
	"auto / auto re i think",
	"unauto / turns off auto re",
	"killaura - activa kill aura ",
	"nokillaura / takes off kill aura",
	"view [plr]/ views player",
	"unview / stops viewing player",
	"fastpunch / rapid punch",
	"slowpunch / normal punch",
	"superpunch / one punch",
	...
	"hipheight [number]",
	"gravity [number]",
	"resetspeed / resets speed",
	"resetjumppower / resets jump power",
	"resethipheight / resets hip hight",
	"resetgravity / resets gravity",
	"makecrim [plr] - makes player into a crim",
	"makecrimall - crims all",
	"loopbring [plr] - loop brings player",
	"unloopbring [plr] - unloop brings player",
	...
	"gun / guns / gives you all the guns",
	"autogun / autoarmas /  automaticly gives guns",
	"unautogun / unautoguns / takes off aauto give guns",
	"taserbypass / antitaser / not get tased",
	"untaserbypass / notaserbypass / turns off taser bypass",
	"nodoors / take off doors - delete doors",
	"restoredoors / put back doors - puts back doors",
	"nowalls / walls off - no walls",
	"walls / restorewalls - puts walls back",
	"anticrash / antivest - Anti crash from armor",
	"unanticrash / unantivest - anticrash ",
	"antishield / noshield - anti swat",
	"unantishield - turns off anti swat shield",
	"gatetower - tp to gate tower",
	"tower - tp to tower",
	"sewer - tp to sewer",
	"backnexus - hall way of prison",
	"nexus - tp to prison",
	"gate - tp to prison out side gate",
	"armory - tp to armory",
	"cafe - Teletransportar a Cafe",
	"crimbase / criminalbase - tp to crim base",
	"lunchroom - tp to lunch room",
	"spamchat [delay] - Spams chat",
	"unspamchat - stops chat spam",
	"savepos / saveposition - save position to load",
	"loadpos / loadposition - load position",
	"notify -  Notificacion/ player leaves or joins",
	"nonotify - unnotify",
	"opengate - opens front gate",
	"antifell / antivoid - Antivoid (samething as anti void)",
	"unantifell / unantivoid - when you have auto re you dont get voided",
	"lag  - lags server forever",
	"prefix is ;",
}
	...
	if Player and Distance then
		...
		RunService:BindToRenderStep(Player.Name, math.huge, function()
			coroutine.wrap(function()
				CreateBeam(Player, Distance, Character.HumanoidRootPart.CFrame)
			end)()
		end)
		delay(Time, function()
			pcall(function()
...
if not Slient then
	Chat("Tubers admin: A server head admin has joined This server!")
	wait()
	Chat("Tubers admin: the head admin does not want you to be begging for admin so please be nice to him!")
end
...
function PlayerChatted(Message)
	...
	if Command("lag") then
		...
		if Player then
			Beam(Player, math.huge, tonumber(Arg3) or 3)
		end
	end
	...
	if Command("antivest") or Command("anticrash") then
		...
		while wait() do
			for i,v in pairs(game.Players:GetPlayers()) do
				pcall(function()
					if workspace[v.Name]:FindFirstChild("vest") then
						workspace[v.Name]:FindFirstChild("vest"):Destroy()
					end
				end)
			end
		end
	...
	if Command("loopbring") then
		...
		if Player ~= nil then
			...
			while wait() do
				pcall(function()
					if States.LoopBring and game.Players[Player.Name] then
						local savedcf = GetPos()
	...
	if Command("admin") or Command("giveadmin") then
		...
		if Player ~= nil and not Admin[Player.UserId] then
			...
			Chat("/w "..Player.Name.." the head admin ranked you admin! type "..Prefix.."cmds to see commands you prefix will be "..Prefix.." type cmds to see commands")
		end
	end
	if Command("unadmin") or Command("removeadmin") then
		...
		if Player ~= nil and Admin[Player.UserId] then
			...
			Chat("/w "..Player.Name.." your admin commands were REMOVED")
		end
	end
...
function AdminPlayerChatted(Message, Player)
	...
	if Command("bring") then
		Teleport(GetPlayer(Arg2), Player.Character.HumanoidRootPart.CFrame)
	end
	if Command("void") then
	...
	if Command("cmd") or Command("cmds") then
		...
		Chat("/w "..Player.Name.." "..Prefix.."cmd "..Prefix.."cmds and the head admin will be watching your commands") wait(.1)
	end
end
